""" Адресна книга з підтримкою днів народження та збереженням даних у файл. """
...
class Phone(Field):
    """ Клас для зберігання номера телефону контакту. """
    def __init__(self, value):
        self.__value = None
        self.value = value
...
class AddressBook(UserDict):
    """ Клас для зберігання колекції контактів. """
    ...
    def get_upcoming_birthdays(self, days=7) -> list:
        """ Повертає список контактів, у яких день народження в найближчі дні. """
...
def add_contact(args, book: AddressBook):
    """ Додає контакт у адресну книгу. Якщо контакт вже існує, додає новий номер телефону. """
    name, phone, *_ = args
...
def show_phone(args, book):
    """ Показує всі номери телефону контакту. """
    (name,) = args
    record = book.find(name)
    if record:
...
def load_data():
    """ Завантажує дані з файлу, якщо він існує, або створює нову адресну книгу. """
...
def main():
    """ Основна функція програми, яка обробляє введені користувачем команди. """
    book = load_data()
